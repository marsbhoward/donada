...
(function (): Contract {
    ...
    const drawDate: Timeout = TimeParam("Timeout for Ada deposit");
    

    // NFT and Contract Details
    const policyId = "85bb65085bb65085bb65085bb65085bb65085bb65085bb65085bb650";  // NFT Policy ID
    const assetName = "Donada";  // NFT Asset Name
    ...
    const nftToken = Token(policyId, assetName);  // NFT as a token 
        ...
        function makePayment(src: payment, timeout: Timeout,
...
const payRentalFee = Pay(
  ...
  Close // Continuation action: Close the contract after payment
);
    ...
    const isRentalFeeChoiceId: ChoiceId = {choice_name: "isRentalFee", choice_owner: nftOwner}

    const checkIfRentalFee = Choice(
        isRentalFeeChoiceId,
        [Bound(0,1)]
    );

    const nftIsRented = When(
        [
            Case(
                Notify(TrueObs),
                 Pay(
    ...
    Case(depositNFT,
      When(
        [
          ...
          Case(
            checkIfRentalFee,  // Use payRentalFee directly as an action
            When(
              [
 ...
 const fullContract = When(
  [
    Case(Notify(TrueObs), returnNFT) // Use Notify as a trigger for returning the NFT
  ],
      TimeParam("drawDate"),
      contract
    );
